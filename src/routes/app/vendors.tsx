...
function Vendors() {
  ...
  const save = useMutation({
    mutationFn: () => saveVendor({ data: { name, trade, contactEmail: email } }),
    onSuccess: () => {
      setName("");
      setTrade("");
      setEmail("");
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={onSubmit} className="grid gap-3 border border-border bg-surface p-4 md:grid-cols-4">
        <Field label="Company">
        ...
        {save.error ? <p className="md:col-span-4 text-sm text-bad">{save.error.message}</p> : null}
      </form>
        ...
        {q.data?.map((v) => (
          <li key={v.id} className="flex items-center justify-between bg-surface px-4 py-3">
            <div>
              ...
              <div className="text-xs text-muted">{v.trade || "Trade unset"} {v.contact_email ? "· " + v.contact_email : ""}</div>
            </div>
